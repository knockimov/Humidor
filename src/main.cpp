...
char token[40] = "";
char name[40] = "Humidor001";
uint32_t update = 120000;
...
void StartWifiConfig()
{
  if ((digitalRead(TRIGGER_PIN) == LOW) || (initialConfig))   // is configuration portal requested?
  {
    ...
    WiFiManagerParameter p_token("token", "API Key", token, 40);
    WiFiManagerParameter p_name("name", "Device Name", name, 40);
    WiFiManagerParameter p_update("update", "Update Intervall (ms)", String(update).c_str(), 8, TYPE_NUMBER);
...
void loop()
{
  ...
  if (currentMillisUpdate - previousMillisUpdate >= update)
  {
